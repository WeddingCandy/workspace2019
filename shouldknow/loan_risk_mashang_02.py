...
if __name__ == '__main__':
    ...
    bank_name_setlen = train_bankcard.groupby(by=['id'], as_index=False)['bank_name'].agg(
    ...
    bank_phone_num_setlen = train_bankcard.groupby(by=['id'], as_index=False)['phone'].agg(
        {'bank_phone_num': lambda x: x.nunique()})

    train_bankcard['card_type_score'] = train_bankcard['card_type'].map(
        lambda x: 0.0154925 if x == '信用卡' else 0.02607069)
    bank_card_type_score = train_bankcard.groupby(by=['id'], as_index=False)['card_type_score'].agg(
        {'card_type_score_mean': np.mean})
    ...
    train_order['amt_order_ordertable'] = train_order['amt_order'].map(
        lambda x: np.nan if ((x == 'NA') | (x == 'null')) else float(x))
    train_order['unit_price_ordertable'] = train_order['unit_price'].map(
        lambda x: np.nan if ((x == 'NA') | (x == 'null')) else float(x))

    train_order['time_order_ordertable'] = train_order['time_order'].map(
    ...
    train_data['是否认证时间在借贷时间前'] = train_data.apply(
        lambda x: 0 if (x['is_auth_time_authtable'] == 0) else (1 if x['auth_time'] < x['appl_sbm_tm'] else 0), axis=1)
    train_data['是否认证时间在借贷时间后'] = train_data.apply(
        lambda x: 0 if (x['is_auth_time_authtable'] == 0) else (1 if x['auth_time'] > x['appl_sbm_tm'] else 0), axis=1)
    train_data['认证时间在借贷时间前多少天'] = train_data.apply(
        lambda x: 0 if (x['是否认证时间在借贷时间前'] == 0) else (x['appl_sbm_tm'] - x['auth_time']).days, axis=1)
    train_data['认证时间在借贷时间后多少天'] = train_data.apply(
        lambda x: 0 if (x['是否认证时间在借贷时间后'] == 0) else (x['auth_time'] - x['appl_sbm_tm']).days, axis=1)
    ...
    tmp_train_order_before_appl_sbm_tm = tmp_train_order[
        tmp_train_order.time_order_ordertable < tmp_train_order.appl_sbm_tm]
    tmp_train_order_after_appl_sbm_tm = tmp_train_order[
        tmp_train_order.time_order_ordertable > tmp_train_order.appl_sbm_tm]
    before_appl_sbm_tm_howmany = tmp_train_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买': len})
    after_appl_sbm_tm_howmany = tmp_train_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买': len})
    before_appl_sbm_tm_money_mean = tmp_train_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买': np.mean})
    after_appl_sbm_tm_money_mean = tmp_train_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买': np.mean})
    before_appl_sbm_tm_money_max = tmp_train_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买最大值': np.max})
    after_appl_sbm_tm_money_min = tmp_train_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买最小值': np.min})
    ...
    if IS_OFFLine == False:
        ...
        train_data = train_data.drop(['appl_sbm_tm', 'id', 'auth_time', 'phone', 'birthday', 'hobby', 'id_card'],
                                     axis=1)
    ...
    if IS_OFFLine == True:
        ...
        valid_train_train = valid_train_train.drop(
            ['appl_sbm_tm', 'id', 'auth_time', 'phone', 'birthday', 'hobby', 'id_card'], axis=1)
    ...
    # 注册时是否有时间
    test_auth['is_auth_time_authtable'] = test_auth['auth_time'].map(
        lambda x: 0 if ((str(x) == 'nan') | (str(x) == '0000-00-00')) else 1)
    # 注册时是否有idcard
    ...
    bank_name_setlen = test_bankcard.groupby(by=['id'], as_index=False)['bank_name'].agg(
        {'bank_name_len': lambda x: len(set(x))})
    bank_num_len = test_bankcard.groupby(by=['id'], as_index=False)['tail_num'].agg({'tail_num_len': lambda x: len(x)})
    bank_phone_num_setlen = test_bankcard.groupby(by=['id'], as_index=False)['phone'].agg(
        {'bank_phone_num': lambda x: x.nunique()})
    test_bankcard['card_type_score'] = test_bankcard['card_type'].map(lambda x: 0.0154925 if x == '信用卡' else 0.02607069)
    bank_card_type_score = test_bankcard.groupby(by=['id'], as_index=False)['card_type_score'].agg(
        {'card_type_score_mean': np.mean})
    ...
    test_order['amt_order_ordertable'] = test_order['amt_order'].map(
        lambda x: np.nan if ((x == 'NA') | (x == 'null')) else float(x))
    test_order['unit_price_ordertable'] = test_order['unit_price'].map(
        lambda x: np.nan if ((x == 'NA') | (x == 'null')) else float(x))
    test_order['time_order_ordertable'] = test_order['time_order'].map(
        lambda x: pd.lib.NaT if (str(x) == '0' or x == 'NA' or x == 'nan')
        else (datetime.datetime.strptime(str(x), '%Y-%m-%d %H:%M:%S') if ':' in str(x)
              else (datetime.datetime.utcfromtimestamp(int(x[0:10])) + datetime.timedelta(hours=8))))
    ...
    test_data['是否认证时间在借贷时间前'] = test_data.apply(
        lambda x: 0 if (x['is_auth_time_authtable'] == 0) else (1 if x['auth_time'] < x['appl_sbm_tm'] else 0), axis=1)
    test_data['是否认证时间在借贷时间后'] = test_data.apply(
        lambda x: 0 if (x['is_auth_time_authtable'] == 0) else (1 if x['auth_time'] > x['appl_sbm_tm'] else 0), axis=1)
    test_data['认证时间在借贷时间前多少天'] = test_data.apply(
        lambda x: 0 if (x['是否认证时间在借贷时间前'] == 0) else (x['appl_sbm_tm'] - x['auth_time']).days, axis=1)
    test_data['认证时间在借贷时间后多少天'] = test_data.apply(
        lambda x: 0 if (x['是否认证时间在借贷时间后'] == 0) else (x['auth_time'] - x['appl_sbm_tm']).days, axis=1)
    ...
    tmp_test_order_before_appl_sbm_tm = tmp_test_order[
        tmp_test_order.time_order_ordertable < tmp_test_order.appl_sbm_tm]
    tmp_test_order_after_appl_sbm_tm = tmp_test_order[tmp_test_order.time_order_ordertable > tmp_test_order.appl_sbm_tm]
    before_appl_sbm_tm_howmany = tmp_test_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买': len})
    after_appl_sbm_tm_howmany = tmp_test_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买': len})
    before_appl_sbm_tm_money_mean = tmp_test_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买': np.mean})
    after_appl_sbm_tm_money_mean = tmp_test_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买': np.mean})
    before_appl_sbm_tm_money_max = tmp_test_order_before_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间前有多少次购买最大值': np.max})
    after_appl_sbm_tm_money_min = tmp_test_order_after_appl_sbm_tm.groupby(by=['id'], as_index=False)[
        'amt_order_ordertable'].agg({'借贷时间后有多少次购买最小值': np.min})
